...
import { validateInput, extractContent } from "../_shared/requestHelpers.ts";
...
async function reportError(err, tags = {}) {
  ...
  try {
    ...
    await fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        message,
        tags: {
          service: "chat-parenting-assistant",
          ...tags
        },
...
async function checkScriptQuota(userId) {
...
async function logSafetyEvent(data) {
  ...
  try {
    ...
    console.warn("[STURDY_SAFETY] Failed to log");
  }
}
async function logUsageEvent(data) {
  ...
  try {
    ...
    console.warn("[STURDY_USAGE] Failed to log");
  }
}
async function logInteractionEvent(data) {
  ...
  try {
    ...
    console.warn("[STURDY_INTERACTION] Failed to log");
  }
}
...
async function generateQuestionResponse(prompt) {
  if (!ANTHROPIC_API_KEY) throw new Error("Missing ANTHROPIC_API_KEY.");
  const controller = new AbortController();
  const timeoutId = setTimeout(()=>controller.abort(), 30000);
  try {
...
serve(async (req)=>{
  ...
  if (input.mode === 'question') {
    try {
      ...
      const result = await generateQuestionResponse(questionPrompt);
  ...
  try {
    ...
    return jsonRes({
      error: "Couldn't generate a script right now."
    }, 500);
  }
});
