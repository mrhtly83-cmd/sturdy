...
function isValidFollowUpResponse(v: unknown): v is FollowUpResponse {
  if (!v || typeof v !== 'object') return false;
  const c = v as Record<string, unknown>;
  return (
    typeof c.followup_type === 'string' &&
    typeof c.title         === 'string' &&
    typeof c.what_happened === 'string' &&
    typeof c.what_to_do    === 'string' &&
    typeof c.why_it_works  === 'string' &&
    typeof c.say_this      === 'string'
  );
}
...
export async function getFollowUpResponse(
  ...
  console.log('[API] Sending follow-up request — type:', input.followUpType);
  ...
  const inner = typeof data === 'object' && data !== null && 'response_type' in data
    ? data as Record<string, unknown>
    : data;


  if (!isValidFollowUpResponse(inner)) {
    console.log('[API] Follow-up validation failed:', JSON.stringify(data).slice(0, 300));
    throw new Error('invalid-followup-response');
  }


  console.log('[API] Follow-up success');
  return inner as FollowUpResponse;
}
