...
// v7 — Journal identity: pastel gradient, frosted glass, rose CTA
// v7 change: removed router.replace — AuthGate in _layout.tsx now owns post-signup routing
...
export default function SignUpScreen() {
 ...
 const handleSignUp = async () => {
   ...
   try {
     ...
     // Legacy pending-child flow (kept for safety — writes any queued
     // child profile from guest mode). No-op when guest mode is disabled.
     try {
     ...
     // No router.replace here — AuthGate in _layout.tsx handles routing:
     //   new session + no child → /child-setup
     //   new session + has child (from pending flow) → /(tabs)
   } catch (err: any) {
