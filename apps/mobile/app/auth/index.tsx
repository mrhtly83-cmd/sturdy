// app/auth/index.tsx
// v5 — Twilight × Obsidian Gold. Theme only — structure identical to v4.
//
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isSignup) {
        ...
        // Session present (confirmation disabled) — navigate explicitly.
        router.replace('/(tabs)');
      } else {
        ...
        <View style={s.stickyContent}>
          ...
          >
            {(!canSubmit || loading) ? (
              ...
              <LinearGradient
                colors={[C.amber, C.amberMid, '#8a6820']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
                style={s.ctaBase}
              >
